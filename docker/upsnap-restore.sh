...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/upsnap"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/upsnap"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/upsnap-restore"
    mkdir -p "$TEMP_DIR"
    tar -xzf "$BACKUP_FILE_PATH" -C "$TEMP_DIR"

    # Stop the upsnap container
    docker stop "$CONTAINER_NAME"

    # Copy the backup files to the host volume directory
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/db" "$HOST_VOLUME_DIR"

    # Remove the temporary directory
    rm -rf "$TEMP_DIR"
    
    # Start the upsnap container
