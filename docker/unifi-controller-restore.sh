...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/unifi-controller"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/unifi-controller"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/unifi-controller-restore"
    mkdir -p "$TEMP_DIR"
    tar -xzf "$BACKUP_FILE_PATH" -C "$TEMP_DIR"

    # Stop the unifi-controller container
    ...
    # Start the unifi-controller container
