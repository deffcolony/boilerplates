...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/uptimekuma"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/uptimekuma"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/uptimekuma-restore"
    mkdir -p "$TEMP_DIR"
    tar -xzf "$BACKUP_FILE_PATH" -C "$TEMP_DIR"

    # Stop the uptimekuma container
    ...
    # Start the uptimekuma container
