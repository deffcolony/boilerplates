...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/dolibarr"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/dolibarr"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/dolibarr-restore"
    mkdir -p "$TEMP_DIR"
    tar -xzf "$BACKUP_FILE_PATH" -C "$TEMP_DIR"

    # Stop the dolibarr container
    docker stop "$CONTAINER_NAME"

    # Copy the backup files to the host volume directory
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/documents" "$HOST_VOLUME_DIR"
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/html/custom" "$HOST_VOLUME_DIR"

    # Restore the database from the SQL dump file
    docker exec -i "$DB_CONTAINER_NAME" sh -c 'exec mysql -uroot -p"$MYSQL_ROOT_PASSWORD"' < "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/dolibarr-db.sql"

    # Remove the temporary directory
    rm -rf "$TEMP_DIR"
    
    # Start the dolibarr container
