...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/filebrowser"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/filebrowser"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/filebrowser-restore"
    mkdir -p "$TEMP_DIR"
    tar -xzf "$BACKUP_FILE_PATH" -C "$TEMP_DIR"

    # Stop the filebrowser container
    docker stop "$CONTAINER_NAME"

    # Copy the backup files to the host volume directory
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/srv" "$HOST_VOLUME_DIR"
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/database/filebrowser.db" "$HOST_VOLUME_DIR"
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/config/filebrowser.json" "$HOST_VOLUME_DIR"
