...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/hpage"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/hpage"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/hpage-restore"
    mkdir -p "$TEMP_DIR"
    tar -xzf "$BACKUP_FILE_PATH" -C "$TEMP_DIR"

    # Stop the hpage container
    docker stop "$CONTAINER_NAME"

    # Copy the backup files to the host volume directory
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/images" "$HOST_VOLUME_DIR"
