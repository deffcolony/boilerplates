...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/kasm"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/kasm"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/kasm-restore"
    mkdir -p "$TEMP_DIR"
    tar -xzf "$BACKUP_FILE_PATH" -C "$TEMP_DIR"

    # Stop the kasm container
    ...
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/profiles" "$HOST_VOLUME_DIR"

    # Remove the temporary directory
    rm -rf "$TEMP_DIR"
    
    # Start the kasm container
