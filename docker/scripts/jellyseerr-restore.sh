...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/jellyseerr"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/jellyseerr"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/jellyseerr-restore"
    mkdir -p "$TEMP_DIR"
    ...
    # Start the jellyseerr container
