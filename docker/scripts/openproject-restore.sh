...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/openproject"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/openproject"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/openproject-restore"
    mkdir -p "$TEMP_DIR"
    tar -xzf "$BACKUP_FILE_PATH" -C "$TEMP_DIR"

    # Stop the openproject container
    ...
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/assets" "$HOST_VOLUME_DIR"

    # Remove the temporary directory
    rm -rf "$TEMP_DIR"
    
    # Start the openproject container
