...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/guacamole"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/guacamole"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/guacamole-restore"
    mkdir -p "$TEMP_DIR"
    ...
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/postgres" "$HOST_VOLUME_DIR"
