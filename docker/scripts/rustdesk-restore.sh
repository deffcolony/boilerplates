...
# Set the rustdesk-hbbs container name
CONTAINER_NAME="rustdesk-hbbs"
HBBR_CONTAINER_NAME="rustdesk-hbbr"

# Set the backup directory
BACKUP_DIR="/home/gebruikersnaam/docker/backups/rustdesk-hbbs"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/rustdesk-hbbs"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/rustdesk-hbbs-restore"
    mkdir -p "$TEMP_DIR"
    tar -xzf "$BACKUP_FILE_PATH" -C "$TEMP_DIR"

    # Stop the rustdesk-hbbs container
    docker stop "$CONTAINER_NAME"
    docker stop "$HBBR_CONTAINER_NAME"

    # Copy the backup files to the host volume directory
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/hbbs/data" "$HOST_VOLUME_DIR"
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/hbbr/data" "$HOST_VOLUME_DIR"

    # Remove the temporary directory
    rm -rf "$TEMP_DIR"
    
    # Start the rustdesk-hbbs container
    docker start "$CONTAINER_NAME"
    docker start "$HBBR_CONTAINER_NAME"
