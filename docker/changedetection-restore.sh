...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/changedetection"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/changedetection"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/changedetection-restore"
    mkdir -p "$TEMP_DIR"
    tar -xzf "$BACKUP_FILE_PATH" -C "$TEMP_DIR"

    # Stop the changedetection container
    docker stop "$CONTAINER_NAME"

    # Copy the backup files to the host volume directory
    cp -r "$TEMP_DIR/$(basename "$BACKUP_FILE_PATH" .tar.gz)/datastore" "$HOST_VOLUME_DIR"
    
    # Remove the temporary directory
    rm -rf "$TEMP_DIR"
    
    # Start the changedetection container
