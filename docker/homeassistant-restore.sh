...
BACKUP_DIR="/home/gebruikersnaam/docker/backups/homeassistant"

# Set the directory where the host volumes are mounted
HOST_VOLUME_DIR="/home/gebruikersnaam/docker/homeassistant"
...
while true; do
  ...
  if [ -z "$CHOICE" ]; then
    ...
    TEMP_DIR="/tmp/homeassistant-restore"
    mkdir -p "$TEMP_DIR"
    ...
    # Start the homeassistant container
